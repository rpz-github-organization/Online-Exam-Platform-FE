...
export default {
  ...
  methods: {
    async submitupdata() { // 获取登录学生的个人信息
      try {
    ...
    async editStudent() { // 修改基本信息
      try {
        ...
        const res = await this.$axios.post(`${this.HOST}/PersonalData/editStudentBaseData`, {
    ...
    async updataEmail() { // 更新邮箱
      try {
        const res = await this.$axios.post(`${this.HOST}/PersonalData/updateStudentNewEmail`, {
          code: this.newNumber,
    ...
    async upataPwd() { // 更新密码
      try {
        const res = await this.$axios.post(`${this.HOST}/PersonalData/updateStudentPassword`, {
    ...
    async submitcheck() { // 发送旧邮箱验证码
      try {
        if (this.email.search('@sicnu.edu.cn') === -1) {
          const res = await this.$axios.post(`${this.HOST}/PersonalData/checkStudentEmail `, {
            email: this.email,
          });
          const info = res.data;
          if (info.code === 200) {
